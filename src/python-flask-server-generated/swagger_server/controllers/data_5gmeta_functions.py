...
from swagger_server.models.mec_instance import MECInstance  # noqa: E501
from swagger_server.models.mec_instance_list import MECInstanceList  # noqa: E501
from swagger_server import util

from swagger_server.models.geolocation_tile import GeolocationTile
from swagger_server.models.geolocation_tile_list import GeolocationTileList
from swagger_server.models.sb_service import SBService
...
try:
  ...
  if err.errno == errorcode.ER_ACCESS_DENIED_ERROR:
    ...
    print("CONNECTION SUCESS " + str(cnx))
       
    cursor = cnx.cursor()

    query = ("SELECT * FROM app_db.MECservers " )
            # "WHERE hire_date BETWEEN %s AND %s")

    cursor.execute(query)#, (hire_start, hire_end))

    result = []
    print("Currently in the DB " + str(cursor))
    for (id, name, lat, lng, props) in cursor:
        print("ID: {} Name: {} lat: {} lon: {} ".format(id, name, lat, lng))
        result.append(MECInstance(id, "VICOM", 
          [SSBervice("MEC", Config("g5meta-project", "1.2.3.4", 
            [Endpoint("/5gmeta/mec/login", "post", ["user", "pass"])]))]
          , GeoValidity()))
    cursor.close()
